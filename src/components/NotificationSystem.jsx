...
const NotificationItem = ({ notification, onRemove }) => {
  ...
  const getBackgroundColor = () => {
    switch (notification.type) {
      case NOTIFICATION_TYPES.SUCCESS:
        return 'bg-slate-900/90 border-l-4 border-green-500 shadow-green-500/20';
      case NOTIFICATION_TYPES.ERROR:
        return 'bg-slate-900/90 border-l-4 border-red-500 shadow-red-500/20';
      case NOTIFICATION_TYPES.WARNING:
        return 'bg-slate-900/90 border-l-4 border-yellow-500 shadow-yellow-500/20';
      case NOTIFICATION_TYPES.INFO:
        return 'bg-slate-900/90 border-l-4 border-blue-500 shadow-blue-500/20';
      default:
        return 'bg-slate-900/90 border-l-4 border-gray-500 shadow-gray-500/20';
    }
  };
  ...
  return (
    <div className={`${getBackgroundColor()} rounded-lg p-4 shadow-xl backdrop-blur-md transition-all duration-300 notification-enhanced ${isVisible ? 'animate-slideInRight' : 'animate-slideOutRight'}`}>
      <div className="flex items-start justify-between">
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-white mb-2 leading-relaxed opacity-90">
              {notification.message}
            </p>
            {notification.details && (
              <div className="text-xs text-slate-300 bg-white/10 rounded px-2 py-1">
                {notification.details}
              </div>
            )}
        ...
        <button
          ...
          className="flex-shrink-0 text-slate-400 hover:text-white transition-colors hover:bg-white/10 rounded p-1"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
