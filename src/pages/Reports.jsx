...
const Reports = () => {
  ...
  const analyzeCustomerData = (sales) => {
    ...
    sales.forEach(sale => {
      ...
      customerData[customerName].totalSpent = safeMath.add(customerData[customerName].totalSpent, sale.total || 0);
      ...
      const confirmDelete = () => {
        try {
          ...
          if (activeShift) {
            ...
            activeShift.totalSales = safeMath.subtract(activeShift.totalSales, invoice.total || 0);
  ...
  const increaseItemQuantity = (invoiceId, itemIndex) => {
    try {
      ...
      if (invoice && invoice.items[itemIndex]) {
        ...
        // إعادة حساب المبالغ
        updatedInvoice.subtotal = safeMath.calculateSubtotal(updatedInvoice.items);
        const taxableAmount = Math.max(0, safeMath.subtract(updatedInvoice.subtotal, updatedInvoice.discountAmount || 0));
        updatedInvoice.total = Math.max(0, safeMath.add(taxableAmount, updatedInvoice.taxAmount || 0));
        ...
        try {
          ...
          if (activeShift && activeShift.status === 'active' && updatedInvoice.shiftId === activeShift.id) {
            activeShift.sales = activeShift.sales.map(s => s.id === invoiceId ? updatedInvoice : s);
            localStorage.setItem('activeShift', JSON.stringify(activeShift));
          }
        } catch (_) { }
  ...
  const generateInvoiceContent = (invoice) => {
    ...
    const subtotal = invoice.subtotal || safeMath.calculateSubtotal(invoice.items || []);
    const total = invoice.total || safeMath.add(Math.max(0, safeMath.subtract(subtotal, invoice.discountAmount || 0)), invoice.taxAmount || 0);

    // حساب المبلغ المتبقي
    const remainingAmount = invoice.downPayment && invoice.downPayment.enabled
      ? safeMath.subtract(total, invoice.downPayment.amount || 0)
      : total;
      ...
      <html dir="rtl">
        <head>
          ...
          <style>
            ...
            table.items-table th.name { width: 45%; }
            table.items-table th.qty { width: 15%; text-align: center; }
            table.items-table th.price { width: 20%; text-align: center; }
            table.items-table th.total { width: 20%; text-align: center; }
  ...
  const openSettlementModal = (invoiceId) => {
    ...
    const remainingAmount = invoice.downPayment.remaining || (safeMath.subtract(invoice.total, invoice.downPayment.amount));
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6 lg:space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center  space-y-4 md:space-y-0">
          ...
          <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
            ...
            <button
              ...
              className="bg-gradient-to-r from-red-500 to-pink-500 text-slate-800 px-3 md:px-4 py-2 md:py-3 rounded-xl hover:from-red-600 hover:to-pink-600 transition-all duration-300 flex items-center text-xs md:text-xs lg:text-sm font-semibold shadow-lg min-h-[40px] cursor-pointer"
              style={{
                pointerEvents: 'auto',
                zIndex: 10,
                position: 'relative'
            ...
            <button
              ...
              className="bg-gradient-to-r from-green-500 to-emerald-500 text-slate-800 px-3 md:px-4 py-2 md:py-3 rounded-xl hover:from-green-600 hover:to-emerald-600 transition-all duration-300 flex items-center text-xs md:text-xs lg:text-sm font-semibold shadow-lg min-h-[40px] cursor-pointer"
        ...
        <div className="glass-card p-4 md:p-6 " style={{ animationDelay: '0.1s' }}>
            ...
            {reports.map((report) => {
              ...
              return (
                <button
                  ...
                  className={`p-3 md:p-4 rounded-xl md:rounded-lg border-2 flex flex-col items-center transition-all duration-300 ${selectedReport === report.id
                ...
                >
                  <Icon className="h-5 w-5 md:h-6 md:w-6 mb-2 text-slate-800" />
                  <span className="text-xs md:text-sm font-medium text-slate-800">{report.name}</span>
                </button>
              );
            })}
          </div>
        ...
        {/* Period Selection */}
        <div className="glass-card p-4 md:p-6  mb-4 md:mb-6" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="flex flex-wrap gap-2 md:gap-3">
            {periods.map((period) => (
              <button
                ...
                className={`px-3 md:px-4 py-2 rounded-lg border transition-colors ${selectedPeriod === period.id
                  ? 'border-blue-500 bg-blue-500 bg-opacity-20 text-blue-300 shadow-glow'
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 ipad-grid ipad-pro-grid gap-4 md:gap-6 mb-4 md:mb-6">
          <div className="glass-card hover-lift  group cursor-pointer p-4 md:p-6" style={{ animationDelay: '0.3s' }}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex-1">
                <p className="text-xs font-medium text-slate-200 mb-1 uppercase tracking-wide">إجمالي المبيعات</p>
                <p className="text-lg md:text-xl font-bold text-slate-800 mb-2">{(realTimeData?.dailySales?.reduce((s, d) => s + (Number(d.sales) || 0), 0) || 0).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</p>
                <div className="flex items-center text-xs">
                  <TrendingUp className="h-3 w-3 text-green-400 mr-1" />
                  <span className="text-green-400 font-semibold">+12.5%</span>
                  <span className="text-slate-500 mr-1 font-medium">من الفترة الماضية</span>
                </div>
              </div>
              <div className="p-2 md:p-3 bg-gradient-to-r from-green-500 to-emerald-500 rounded-xl md:rounded-2xl group-hover:scale-110 transition-transform duration-300 shadow-lg">
                <DollarSign className="h-5 w-5 md:h-6 md:w-6 text-slate-800" />
              </div>
            </div>
          </div>

          <div className="glass-card hover-lift  group cursor-pointer p-4 md:p-6" style={{ animationDelay: '0.4s' }}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex-1">
                <p className="text-xs font-medium text-slate-200 mb-1 uppercase tracking-wide">عدد الطلبات</p>
                <p className="text-lg md:text-xl font-bold text-slate-800 mb-2">{(visibleInvoices?.length || 0).toLocaleString('en-US')}</p>
                <div className="flex items-center text-xs">
                  <TrendingUp className="h-3 w-3 text-green-400 mr-1" />
              ...
              <div className="p-2 md:p-3 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-xl md:rounded-2xl group-hover:scale-110 transition-transform duration-300 shadow-lg">
                <ShoppingCart className="h-5 w-5 md:h-6 md:w-6 text-slate-800" />
              </div>
            </div>
          </div>

          <div className="glass-card hover-lift  group cursor-pointer p-4 md:p-6" style={{ animationDelay: '0.5s' }}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex-1">
                <p className="text-xs font-medium text-slate-200 mb-1 uppercase tracking-wide">متوسط قيمة الطلب</p>
                <p className="text-lg md:text-xl font-bold text-slate-800 mb-2">{(() => { const total = allSales.reduce((s, inv) => s + (Number(inv.total) || 0), 0); const orders = allSales.length || 1; return (total / orders).toLocaleString('en-US', { style: 'currency', currency: 'USD' }); })()}</p>
                <div className="flex items-center text-xs">
                  <TrendingUp className="h-3 w-3 text-green-400 mr-1" />
              ...
              <div className="p-2 md:p-3 bg-gradient-to-r from-purple-500 to-violet-500 rounded-xl md:rounded-2xl group-hover:scale-110 transition-transform duration-300 shadow-lg">
                <TrendingUp className="h-5 w-5 md:h-6 md:w-6 text-slate-800" />
              </div>
            </div>
          </div>

          <div className="glass-card hover-lift  group cursor-pointer p-4 md:p-6" style={{ animationDelay: '0.6s' }}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex-1">
                <p className="text-xs font-medium text-slate-200 mb-1 uppercase tracking-wide">عدد العملاء</p>
                <p className="text-lg md:text-xl font-bold text-slate-800 mb-2">{(new Set(allSales.map(inv => (inv.customer?.phone || inv.customer?.name || ''))).size).toLocaleString('en-US')}</p>
                <div className="flex items-center text-xs">
                  <TrendingUp className="h-3 w-3 text-green-400 mr-1" />
              ...
              <div className="p-2 md:p-3 bg-gradient-to-r from-orange-500 to-amber-500 rounded-xl md:rounded-2xl group-hover:scale-110 transition-transform duration-300 shadow-lg">
                <Users className="h-5 w-5 md:h-6 md:w-6 text-slate-800" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 ipad-grid ipad-pro-grid gap-4 md:gap-6 mb-4 md:mb-6">
          {/* Sales Chart */}
          <div className="glass-card hover-lift " style={{ animationDelay: '0.7s' }}>
          ...
          <div className="glass-card hover-lift  chart-enhanced" style={{ animationDelay: '0.8s' }}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-white">توزيع المبيعات حسب التصنيف</h3>
              <div className="p-2 bg-gradient-to-r from-green-500 to-emerald-500 rounded-lg">
                <PieChart className="h-6 w-6 text-slate-800" />
              </div>
            </div>
            <ResponsiveContainer width="100%" height={300}>
              <RechartsPieChart>
        ...
        {/* Data Table */}
        <div className="glass-card hover-lift  overflow-hidden" style={{ animationDelay: '0.9s' }}>
                ...
                <div className="w-full">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                    ...
                    <div className="bg-purple-500 bg-opacity-20 px-4 py-3 rounded-lg border border-purple-500 border-opacity-30">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-slate-500 text-sm">مع العربون</p>
                          <p className="text-slate-800 font-bold text-xl">
                ...
                <thead className="bg-white bg-opacity-10">
                  <tr>
                    ...
                    {selectedReport === 'products' && (
                      <>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">المنتج</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">الكمية المباعة</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">الإيرادات</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">الربح</th>
                      </>
                    )}
                    {selectedReport === 'customers' && (
                      <>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">اسم العميل</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">إجمالي المشتريات</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">عدد الطلبات</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">آخر زيارة</th>
                      </>
                    )}
                    {selectedReport === 'inventory' && (
                      <>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">التصنيف</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">المبيعات (كمية)</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">النسبة%</th>
                      </>
                    )}
                    {(selectedReport === 'invoices' || selectedReport === 'partial-invoices') && (
                      <>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">رقم الفاتورة</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">العميل</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">المبلغ</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">طريقة الدفع</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">التاريخ</th>
                    ...
                    {selectedReport === 'returns' && (
                      <>
                        ...
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">رقم الفاتورة</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">المنتج</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">الكمية</th>
                        <th className="px-4 md:px-6 py-3 text-right text-xs font-medium text-slate-600 uppercase tracking-wider">المبلغ</th>
                      </>
                    )}
                  </tr>
                ...
                <tbody className="divide-y divide-white divide-opacity-20">
                  {(() => {
                      ...
                      return Object.keys(grouped).map(dateKey => (
                        <React.Fragment key={dateKey}>
                          <tr className="bg-gray-700 bg-opacity-40">
                            <td colSpan="6" className="px-4 md:px-6 py-2 text-xs font-semibold text-slate-600">
                              {dateKey}
                            </td>
                          </tr>
                          {grouped[dateKey].map((item, index) => (
                            <tr key={item.id + index} className="hover:bg-slate-200 hover:bg-opacity-20" style={{ position: 'relative' }}>
                              {/* رقم الفاتورة */}
                              ...
                              <td className="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-slate-800">
                                ...
                                {item.downPayment?.enabled && ((item.downPayment?.remaining ?? (safeMath.subtract(item.total, item.downPayment.amount))) > 0) && (
                              ...
                              <td className="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-slate-800">
                                <div className="flex flex-wrap gap-2" style={{ position: 'relative', zIndex: 1, pointerEvents: 'auto' }}>
                                  ...
                                  <button
                                    ...
                                    className="text-purple-400 hover:text-slate-500 transition-all duration-200 p-2 hover:bg-purple-500 hover:bg-opacity-20 rounded-lg border border-purple-400 border-opacity-30 hover:border-opacity-60 min-w-[40px] min-h-[40px] flex items-center justify-center cursor-pointer"
                    ...
                    if (selectedReport === 'products') {
                      return data.map((row, idx) => (
                        <tr key={idx} className="hover:bg-slate-200 hover:bg-opacity-20">
                          <td className="px-4 md:px-6 py-3 text-slate-800">{row.name}</td>
                          <td className="px-4 md:px-6 py-3 text-slate-800">{row.sales}</td>
                          <td className="px-4 md:px-6 py-3 text-green-300 font-semibold">{Number(row.revenue || 0).toLocaleString('en-US')}</td>
                          <td className="px-4 md:px-6 py-3 text-slate-800">{Number(row.profit || 0).toLocaleString('en-US')}</td>
                        </tr>
                      ));
                    }
                    if (selectedReport === 'customers') {
                      return data.map((row, idx) => (
                        <tr key={idx} className="hover:bg-slate-200 hover:bg-opacity-20">
                          ...
                          <td className="px-4 md:px-6 py-3 text-slate-800">{row.orders}</td>
                          <td className="px-4 md:px-6 py-3 text-slate-800">{row.lastVisit}</td>
                        </tr>
                      ));
                    }
                    if (selectedReport === 'inventory') {
                      ...
                      return data.map((row, idx) => (
                        <tr key={idx} className="hover:bg-slate-200 hover:bg-opacity-20">
                          <td className="px-4 md:px-6 py-3 text-slate-800">{row.name}</td>
                          <td className="px-4 md:px-6 py-3 text-slate-800">{Number(row.value || 0).toLocaleString('en-US')}</td>
                          <td className="px-4 md:px-6 py-3 text-slate-800">{Math.round(((Number(row.value) || 0) / totalQty) * 100)}%</td>
                        </tr>
                      ));
                    }
                    if (selectedReport === 'returns') {
                      ...
                      if (!rows || rows.length === 0) {
                        return (
                          <tr>
                            <td colSpan="5" className="px-4 md:px-6 py-12 text-center text-slate-600">لا توجد مرتجعات</td>
                          </tr>
                        );
                      }
                      return rows.map((r, idx) => (
                        <tr key={idx} className="hover:bg-slate-200 hover:bg-opacity-20">
                          <td className="px-4 md:px-6 py-3 text-sm text-slate-800">{formatDateTime(r.timestamp)}</td>
                          <td className="px-4 md:px-6 py-3 text-sm text-slate-800">#{r.refInvoiceId}</td>
                          <td className="px-4 md:px-6 py-3 text-sm text-slate-800">{r.item?.name || ''}</td>
                          <td className="px-4 md:px-6 py-3 text-sm text-slate-800">{r.item?.quantity || 0}</td>
      ...
      {showInvoiceModal && selectedInvoice && selectedInvoice.id && (
        ...
        >
          <div
            className="glass-card p-4 w-full max-w-6xl mx-4 "
            style={{
              position: 'relative',
              zIndex: 10000,
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-white border-opacity-20">
              ...
              <button
                ...
                className="text-slate-300 hover:text-white transition-all duration-200 p-3 hover:bg-red-500 hover:bg-opacity-20 rounded-xl border border-slate-400 hover:border-red-500 min-w-[40px] min-h-[40px] flex items-center justify-center cursor-pointer"
                title="إغلاق النافذة"
                style={{
                  pointerEvents: 'auto',
                  zIndex: 10,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div className="glass-card p-3">
                <h4 className="text-base font-semibold text-slate-800 mb-3">معلومات العميل</h4>
                <div className="space-y-1">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-800 font-medium">{selectedInvoice?.items?.length || 0}</span>
                  </div>
                  <div className="flex justify-between">
              ...
              {/* Payment Info */}
              <div className="glass-card p-3">
                <h4 className="text-base font-semibold text-slate-800 mb-3">معلومات الدفع</h4>
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span className="text-slate-600 text-sm">طريقة الدفع:</span>
                    <div className="flex items-center">
                      {getPaymentMethodIcon(selectedInvoice?.paymentMethod)}
                      <span className="text-slate-800 font-medium mr-2">{getPaymentMethodText(selectedInvoice?.paymentMethod)}</span>
                    </div>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {selectedInvoice?.downPayment && selectedInvoice.downPayment.enabled && (
                    <div className="mt-4 p-4 bg-white bg-opacity-5 rounded-xl border border-white border-opacity-20">
                      <div className="flex items-center mb-3">
                        ...
                        <h5 className="text-slate-800 font-bold text-lg">تفاصيل العربون</h5>
                      </div>
                      <div className="space-y-3">
                        <div className="flex justify-between items-center p-2 bg-white bg-opacity-10 rounded-lg">
                          <span className="text-slate-700 font-medium">المبلغ المدفوع:</span>
                          <span className="text-slate-800 font-bold text-lg">{selectedInvoice?.downPayment?.amount || 0} جنيه</span>
                        </div>
                        <div className="flex justify-between items-center p-2 bg-white bg-opacity-10 rounded-lg">
                          <span className="text-slate-700 font-medium">نوع العربون:</span>
                        ...
                        <div className="flex justify-between items-center p-2 bg-white bg-opacity-10 rounded-lg border border-white border-opacity-10">
                          <span className="text-slate-700 font-medium">المبلغ المتبقي:</span>
                          <span className="text-slate-800 font-bold text-lg">
                            {(() => {
                              const total = selectedInvoice?.total || 0;
                              const downPaymentAmount = selectedInvoice?.downPayment?.amount || 0;
                              const remaining = selectedInvoice?.downPayment?.remaining || (safeMath.subtract(total, downPaymentAmount));
                              return remaining.toFixed(2);
                            })()} جنيه
                          </span>
                        </div>
                        <div className="text-center mt-3">
                          <span className="text-slate-700 text-sm bg-white bg-opacity-10 px-3 py-1 rounded-full border border-white border-opacity-10">
                            فاتورة بعربون
                          </span>
                        </div>
                      </div>
            ...
            <div className="glass-card p-3 mb-4">
              <div className="flex justify-between items-center mb-3">
                <h4 className="text-base font-semibold text-slate-800">المنتجات</h4>
                ...
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
                    <tr className="border-b-2 border-white border-opacity-30 bg-gradient-to-r from-purple-500 to-blue-500 bg-opacity-10">
                      <th className="text-right py-2 px-2 text-slate-600 font-bold text-xs">المنتج</th>
                      <th className="text-right py-2 px-2 text-slate-600 font-bold text-xs">الفئة</th>
                      <th className="text-right py-2 px-2 text-slate-600 font-bold text-xs">الكمية</th>
                      <th className="text-right py-2 px-2 text-slate-600 font-bold text-xs">السعر</th>
                      <th className="text-right py-2 px-2 text-slate-600 font-bold text-xs">المجموع</th>
                      <th className="text-right py-2 px-2 text-slate-600 font-bold text-xs">الإجراءات</th>
                    </tr>
                  </thead>
                  <tbody>
                    {(enrichInvoiceItemsWithCategory(selectedInvoice)?.items || []).map((item, index) => (
                      <tr key={index} className="border-b border-white border-opacity-10 hover:bg-white hover:bg-opacity-5 transition-colors">
                        <td className="py-2 px-2 text-slate-800 font-medium text-xs">{emojiManager.getProductEmoji(item)} {item.name}</td>
                        ...
                        <td className="py-2 px-2 text-slate-800 text-center">
                          <div className="flex items-center justify-center gap-2">
                            <button
                              ...
                              className="bg-red-500 hover:bg-red-600 text-slate-800 px-3 py-2 rounded-full text-sm transition-colors duration-150 select-none"
                              title="تقليل الكمية"
                            >
                              -
                            </button>
                            ...
                            <button
                              ...
                              className="bg-green-500 hover:bg-green-600 text-slate-800 px-3 py-2 rounded-full text-sm transition-colors duration-150 select-none"
                              title="زيادة الكمية"
                            >
                              +
                            </button>
                        ...
                        <td className="py-2 px-2 text-slate-800 font-medium text-xs">{item.price} جنيه</td>
                        <td className="py-2 px-2 text-slate-800 font-bold text-green-400 text-sm">
                          {safeMath.multiply(item.price, item.quantity)} جنيه
                        </td>
            ...
            {/* Totals */}
            <div className="glass-card p-3 mb-4">
              <h4 className="text-base font-semibold text-slate-800 mb-3">تفاصيل المبلغ</h4>
              <div className="space-y-2">
                ...
                {selectedInvoice?.downPayment && selectedInvoice.downPayment.enabled && (
                  <div className="bg-white bg-opacity-5 rounded-lg p-3 space-y-2 border border-white border-opacity-10">
                    <div className="flex items-center mb-1">
                      <DollarSign className="h-3 w-3 text-slate-600 mr-2" />
                      <span className="text-slate-800 font-semibold text-sm">ملخص الدفع</span>
                    </div>
                    <div className="flex justify-between items-center p-1 bg-white bg-opacity-10 rounded-lg">
                      <span className="text-slate-700 font-medium text-sm">العربون المدفوع:</span>
                      <span className="text-slate-800 font-bold text-sm">{selectedInvoice.downPayment.amount} جنيه</span>
                    </div>
            ...
            {/* Actions */}
            <div className="space-y-3">
              <h4 className="text-base font-semibold text-slate-800">الإجراءات المتاحة</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <button
                  ...
                  className="btn-primary bg-gradient-to-r from-red-500 to-pink-500 text-slate-800 px-5 py-4 rounded-2xl hover:from-red-600 hover:to-pink-600 transition-colors duration-150 flex items-center justify-center min-h-[56px] cursor-pointer select-none"
                  style={{
                    pointerEvents: 'auto',
                    zIndex: 10,
                    position: 'relative',
                ...
                <button
                  ...
                  className="btn-primary bg-gradient-to-r from-purple-500 to-indigo-500 text-slate-800 px-5 py-4 rounded-2xl hover:from-purple-600 hover:to-indigo-600 transition-colors duration-150 flex items-center justify-center min-h-[56px] cursor-pointer select-none"
                  style={{
                    pointerEvents: 'auto',
                    zIndex: 10,
                    position: 'relative',
                ...
                {selectedInvoice?.downPayment && selectedInvoice.downPayment.enabled && selectedInvoice.downPayment.remaining > 0 && (
                  <button
                    ...
                    className="btn-primary bg-gradient-to-r from-green-500 to-emerald-500 text-slate-800 px-5 py-4 rounded-2xl hover:from-green-600 hover:to-emerald-600 transition-colors duration-150 flex items-center justify-center min-h-[56px] cursor-pointer select-none"
                    style={{
                      pointerEvents: 'auto',
                      zIndex: 10,
                      position: 'relative',
                ...
                <button
                  ...
                  className="btn-primary bg-gradient-to-r from-red-500 to-pink-500 text-slate-800 px-5 py-4 rounded-2xl hover:from-red-600 hover:to-pink-600 transition-colors duration-150 flex items-center justify-center min-h-[56px] cursor-pointer select-none text-base"
                  style={{
                    pointerEvents: 'auto',
                    zIndex: 10,
                    position: 'relative',
              ...
              <div className="flex justify-center mt-6 pt-4 border-t border-white border-opacity-20">
                <button
                  ...
                  className="bg-gray-500 hover:bg-gray-600 text-slate-800 px-6 py-2 rounded-xl transition-all duration-200 flex items-center min-h-[40px] cursor-pointer"
                  style={{
                    pointerEvents: 'auto',
                    zIndex: 10,
                    position: 'relative'
      ...
      {/* نافذة التأكيد (React State Modal بدلًا من DOM Injection) */}
      {confirmModal.isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4" style={{ zIndex: 100000, pointerEvents: 'auto' }}>
          <div className="glass-card p-6 w-full max-w-md mx-4 text-center">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-red-500 hover:bg-red-600 text-slate-800 px-6 py-3 rounded-xl transition-colors"
              >
                تأكيد
              </button>
              <button
                onClick={closeConfirmModal}
                className="flex-1 bg-gray-500 hover:bg-gray-600 text-slate-800 px-6 py-3 rounded-xl transition-colors"
              >
                إلغاء
              </button>
            </div>
